...
class OneNearestNeighbor(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        ----------------------
        Parameters: 
        X: Data Samples we want to get the label of - ndarray (nb_data_points, nb_features)
        ----------------------
        Returns
        ...
        y_pred = self.y_train_[np.argmin(distance_matrix(X, self.X_train_), axis=1)]
        return y_pred
    ...
    def score(self, X, y):
        ...
        ----------------------
        Returns
        float corresponding to the mean accuracy of the class predicted for data X compared to value y
        """
        X, y = check_X_y(X, y)
